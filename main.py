...
try:
    ...
    print(Fore.YELLOW + "Ban checking will be disabled." + Fore.RESET)
...
def Load():
    global Combos, fname
    filename = input(Fore.LIGHTBLUE_EX+"Combo file path: "+Fore.RESET)
    try:
        ...
        print(Fore.LIGHTRED_EX+"Invalid file path or file is harmed.")
        time.sleep(2)
        Load()

def Proxys():
    global proxylist
    filename = input(Fore.LIGHTBLUE_EX+"Proxy file path: "+Fore.RESET)
    try:
...
def banproxyload():
    global banproxies
    filename = input(Fore.LIGHTBLUE_EX+"SOCKS5 proxy file path: "+Fore.RESET)
    try:
        with open(filename, 'r+', encoding='utf-8', errors='ignore') as e:
            ext = e.readlines()
            for line in ext:
        ...
        print(Fore.LIGHTBLUE_EX+f"Loaded [{len(banproxies)}] SOCKS5 proxies.")
        time.sleep(2)
    except Exception as e:
        print(Fore.LIGHTRED_EX+f"Error reading file: {e}")
        time.sleep(2)
